...
const TEE_PRICE_CENTS = 4500;
const TANK_PRICE_CENTS = 3939;
  ...
  try {
    ...
    for (const item of items) {
      if (!item.product || !item.fit || !item.size || !item.name?.trim()) {
        return NextResponse.json(
          { error: "Each item must have a product, style, size, and name." },
          { status: 400 }
        );
      }
    }
    ...
    const lineItems = items.map((item) => ({
      price_data: {
        currency: "nzd",
        product_data: {
          name: `Mates in Motors ${item.product} — ${item.fit} / ${item.size} / ${item.name}`,
        },
        unit_amount: item.product === "Tee" ? TEE_PRICE_CENTS : TANK_PRICE_CENTS,
      },
      quantity: 1 as const,
    }));
    ...
    console.error("Mates in Motors checkout error:", err);
    return NextResponse.json(
