...
const INTERNAL_EMAIL = process.env.TEAMWEAR_INTERNAL_EMAIL || "ben@tendencies.co.nz";
const FROM_EMAIL = "Tendencies <orders@mail.tendencies.co.nz>";

function formatCurrency(amount?: number | null) {
  if (!amount) return "$0.00 NZD";
  return new Intl.NumberFormat("en-NZ", { style: "currency", currency: "NZD" }).format(amount / 100);
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    event = stripe.webhooks.constructEvent(body, sig, process.env.MIM_STRIPE_WEBHOOK_SECRET!);
  } catch (err) {
    console.error("MIM Stripe webhook verification failed:", err);
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (session.metadata?.team !== "Mates in Motors") {
    ...
    if (sheetsUrl) {
      try {
        ...
        await fetch(sheetsUrl, {
          ...
          body: JSON.stringify({ orderId: session.id, customerName, email: customerEmail || "", phone, notes, items }),
        });
        console.log("MIM ORDER SENT TO SHEETS", session.id);
      } catch (err) {
        console.error("MIM SHEETS LOGGING FAILED", err);
      }
    }
      ...
      <div style="font-family: Helvetica, Arial, sans-serif; background:#080808; color:#f5f5f0; padding:32px;">
        <div style="max-width:640px; margin:0 auto;">
          ...
          <div style="border:1px solid rgba(255,255,255,0.12); background:#0f0f0f; padding:20px; margin-bottom:24px;">
            <div style="font-size:12px; letter-spacing:0.14em; text-transform:uppercase; color:rgba(255,255,255,0.45); margin-bottom:10px;">Order details</div>
    ...
    const internalHtml = `
      <div style="font-family: Helvetica, Arial, sans-serif; padding:24px;">
        <h2>New Mates in Motors Teamwear Order</h2>
    ...
    try {
      if (customerEmail) {
        await resend.emails.send({
          from: FROM_EMAIL,
          to: customerEmail,
          subject: "Mates in Motors — Order Confirmed",
          html: customerHtml,
        });
      }
      await resend.emails.send({
        from: FROM_EMAIL,
        to: INTERNAL_EMAIL,
        subject: `New MIM order — ${customerName}`,
        html: internalHtml,
      });
      console.log("MIM ORDER EMAILS SENT", session.id);
    } catch (err) {
      console.error("MIM failed to send order emails:", err);
    }
  }
