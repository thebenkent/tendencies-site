    ...
    <div>
      ...
      {(campaign.closes_at || totalMinimum > 0) && (
        <div style={{ background: '#fff', borderBottom: '1px solid #E2E8EF' }}>
          <div style={{ maxWidth: '980px', margin: '0 auto', padding: '28px 32px', display: 'grid', gridTemplateColumns: campaign.closes_at ? '1fr auto' : '1fr', gap: '32px', alignItems: 'center' }} className="merch-hero-strip">
            <div>
      ...
      <div style={{ maxWidth: '980px', margin: '0 auto', padding: '48px 24px 80px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '32px', gap: '16px', flexWrap: 'wrap' }}>
          <h2 style={{ fontSize: 'clamp(20px, 2.5vw, 28px)', fontWeight: 800, color: navy, letterSpacing: '-0.025em', margin: 0 }}>
            {campaign.name}
          </h2>
          <a href={`/merch/${slug}/${campaignSlug}/cart`}
            style={{ fontSize: '14px', fontWeight: 600, color: navy, textDecoration: 'none', opacity: 0.6 }}>
            View Cart →
          </a>
        </div>
          ...
          {products.map((product, i) => {
            ...
            return (
              <a
                ...
                style={{ display: 'flex', flexDirection: 'column', background: '#fff', border: '1px solid #E2E8EF', borderRadius: '14px', overflow: 'hidden', textDecoration: 'none', boxShadow: '0 2px 12px rgba(11,31,77,0.07)', transition: 'transform 0.15s, box-shadow 0.15s' }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.transform = 'translateY(-2px)'; (e.currentTarget as HTMLAnchorElement).style.boxShadow = '0 6px 24px rgba(11,31,77,0.13)' }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLAnchorElement).style.transform = ''; (e.currentTarget as HTMLAnchorElement).style.boxShadow = '0 2px 12px rgba(11,31,77,0.07)' }}
              >
                {/* Image */}
                <div style={{ aspectRatio: '1 / 1', background: '#F0F3FA', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                  {product.images[0]
                    ? <Image src={product.images[0]} alt={product.name} fill style={{ objectFit: 'contain', padding: '24px' }} sizes="(max-width: 980px) 50vw, 320px" />
                    : <div style={{ width: '100%', height: '100%', background: `linear-gradient(135deg, ${navy}22 0%, ${red}22 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '64px', opacity: 0.25 }}>👕</div>}
                  {prog.isMet && !closed && (
                    <div style={{ position: 'absolute', top: '12px', left: '12px', background: '#16a34a', color: '#fff', fontSize: '10px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '4px 10px', borderRadius: '4px' }}>Min. Reached ✓</div>
                  )}
                  {closed && (
                    <div style={{ position: 'absolute', top: '12px', left: '12px', background: '#374151', color: '#fff', fontSize: '10px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '4px 10px', borderRadius: '4px' }}>Closed</div>
                  )}
                </div>

                {/* Content */}
                <div style={{ padding: '20px 20px 22px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ fontSize: '19px', fontWeight: 800, color: navy, marginBottom: '6px', letterSpacing: '-0.02em', lineHeight: 1.2 }}>{product.name}</div>
                  <div style={{ fontSize: '14px', color: '#5A6B7E', marginBottom: '20px', lineHeight: 1.55, flex: 1 }}>
                    {(product.description ?? '').slice(0, 90)}{(product.description?.length ?? 0) > 90 ? '…' : ''}
                  </div>

                  {/* Mini progress */}
                  <div style={{ marginBottom: '18px' }}>
                    <div style={{ height: '5px', background: '#E2E8EF', borderRadius: '999px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${Math.min(100, prog.percentage)}%`, background: prog.isMet ? '#16a34a' : `linear-gradient(90deg, ${navy} 0%, ${red} 100%)`, borderRadius: '999px' }} />
                    </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '24px', fontWeight: 800, color: navy, letterSpacing: '-0.02em' }}>${(product.price_cents / 100).toFixed(2)}</span>
                    <span style={{
                      fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase',
                      color: '#fff', background: closed ? '#94A3B8' : red,
